...
namespace FirstWCFClient
{
    public partial class Form1 : Form
    {
        ...
        private void btnTestWCFMyMath_Click(object sender, EventArgs e)
        {
            ...
            IMyMath im = new ChannelFactory<IMyMath>(new BasicHttpBinding(), new EndpointAddress("http://localhost:8700/MM")).CreateChannel();
            double result = im.ComputeAvg(7, 12, 14);
            MessageBox.Show("Result = " + result.ToString());
            ((IChannel)im).Close();
        }

        private void btnTestWCFViaProxy_Click(object sender, EventArgs e)
        {
